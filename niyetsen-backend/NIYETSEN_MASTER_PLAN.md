...
- [ ] Monorepo kur: `backend/`, `mobile/`, `docs/`, kök `CLAUDE.md`, `.gitignore`,
      `NIYETSEN_MASTER_PLAN.md`
- [ ] `backend/`: FastAPI iskeleti (`app/main.py`, `app/api/`, `app/core/`,
      `app/models/`, `app/services/`), `requirements.txt`
      (fastapi, uvicorn, google-genai, httpx, pydantic, slowapi, supabase)
- [ ] `.env.example` doldur (GEMINI_API_KEY, GEMINI_MODEL, SUPABASE_URL,
      SUPABASE_SERVICE_KEY, UNSPLASH_ACCESS_KEY, DATABASE_URL) — gerçek anahtar
      SADECE `.env`'de, asla commit edilmez
- [ ] `mobile/`: `npx create-expo-app` (TypeScript, expo-router), `lib/api.ts`
      fetch sarmalayıcı
- [ ] `GET /health` çalışır; mobil açılış ekranı backend'e ping atar
...
Backend:
- [ ] `core/prompts.py`: SYSTEM_PROMPT (uygulama-promt.md §14 +
      prompts/chat_system_prompt.md TEK dosyada birleştirilmiş hali)
- [ ] `services/gemini_service.py`: chat çağrısı; plan üretiminde
      `response_mime_type="application/json"` + Pydantic şema doğrulaması;
      timeout/retry (§1.9)
- [ ] `POST /chat`: niyet toplama döngüsü — kritik alanlar (şehir, ilgi, zaman)
      dolana kadar soru; 3–4 soruda topla; `ready_for_plan` sinyali
- [ ] `POST /plan/generate`: N günlük JSON plan; her görev 6 kategoriden
      etiketlenir (şimdiden — puan v1'de kullanacak)
- [ ] `services/image_service.py`: image_keyword → Unsplash URL; bulunamazsa
      kategori bazlı yedek görsel
- [ ] `services/plan_service.py`: plan JSON + görselleri birleştir

Mobil:
- [ ] Sohbet ekranı (mesaj listesi + input + yazıyor animasyonu)
- [ ] Plan ekranı (gün gün görselli görev kartları)
- [ ] Hata durumları: ağ yok / Gemini hata → nazik mesaj + tekrar dene
...
- [ ] Supabase projesi kur; §2'deki tablolar migration olarak yazılır
- [ ] Supabase Auth: e-posta + Google + **Apple ile Giriş** (Expo:
      `expo-apple-authentication`, `expo-auth-session`)
- [x] FastAPI JWT middleware: her istek doğrulanır; kullanıcı kimliği token'dan
- [ ] Chat geçmişi + intent + plan DB'ye yazılır; uygulama yeniden açılınca
