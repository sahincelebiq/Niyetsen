...
### Fiziksel cihazdan (Expo Go) test için
Simülatör `localhost`'a erişebilir ama fiziksel telefon aynı Wi-Fi ağındaki
Mac'in LAN IP'sine ihtiyaç duyar. Backend'i şu şekilde başlat:
```bash
uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload
```
Mac'inin LAN IP'sini `ipconfig getifaddr en0` ile bul, mobile projesindeki
`.env` dosyasına `EXPO_PUBLIC_API_URL=http://<LAN_IP>:8000` olarak yaz.
